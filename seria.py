...
#
# 設計方針
# 本ツールは 現地調査での見逃しを避けることを最優先にしています（反応性・確実性 > 省資源）。
# そのため自動検索は取りこぼし防止を優先して対象を広めに取り、短時間実行を前提に設計しています。
# 無通信時は --timeout 間隔で読み取りを繰り返すため、CPU 使用率が上がる場合があります。
# また --wait は「概ねの最大待機時間」を与えるもので、実際の終了時刻は --timeout の設定や
# 読み取り境界の都合でわずかに超過することがあります。
# また、すぐに使える＆簡単に使えることを狙いに1ファイルで構成しています。
# 
# =============================================================================
# 使い方
# =============================================================================
